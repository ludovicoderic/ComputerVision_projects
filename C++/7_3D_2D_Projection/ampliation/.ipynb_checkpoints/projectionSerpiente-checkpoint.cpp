...
int main(int argc, char **argv){
	...
	if(string(argv[1]) == "Snake_by_Swp.pcd"){
   		for (size_t i=0; i<pointCloud->points.size(); i++) { 
    		pts[i].x = (pointCloud->points[i].z/1.2)-20;   //los +X del final es para desplazar en su eje  ////////////15
       		pts[i].y = (pointCloud->points[i].x/1.2)+10;  // los /X es para reducir el tamaño				//////////10
			pts[i].z = -(pointCloud->points[i].y/1.2);    //y los -() son para invertir los puntos sobre su eje  ///////////0
		}
	}else{
	...
	while(1){
    	...
    	if(found){  //si detecta las esquinas del tablero
			...
			//Modificamos la varibla aumento para ir moviendo el origen de la proyeccion de la nube
			if(aumento >= 16){
				aumento = -22;
			}

			//aumento de micropasos para el desplazamiento de la nube
			aumento += 0.255;
			...
			for (size_t i=0; i<pointCloud->points.size(); i++) { 
    		pts[i].x = (pointCloud->points[i].z/1.2)+aumento;
			}
			...
			if(string(argv[1]) == "Snake_by_Swp.pcd"){   //14969
				for(signed i = 0; i<romper; i++){  //point2D.size()
